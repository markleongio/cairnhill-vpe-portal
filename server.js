...
// Expose requireAdmin for routes that need stricter checks
app.locals.requireAdmin = requireAdmin;

// --- Static frontend ---------------------------------------------------------
app.use(express.static(path.join(__dirname, 'public')));

// Public print view doesn't need a session check client-side (the API call
// inside it does, via the same cookie) — just serve the SPA shell for all
// non-API GET requests so client-side routing works.
app.get(/^(?!\/api).*/, (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'index.html'));
});

app.listen(PORT, () => {
  console.log(`Cairnhill VPE Portal running at http://localhost:${PORT}`);
});
