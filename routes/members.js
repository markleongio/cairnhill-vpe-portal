...
router.get('/:id', async (req, res) => {
  try {
    ...
    // Meeting-day duty role history (条件 k): every role this member has
    // held at any meeting (礼宾司/司仪/计时员 etc), distinct from the
    // formal Exco committee history above.
    const meetingRoleHistory = await all(
      'SELECT mra.*, mdr.role_name_zh, mdr.role_name_en, m.meeting_no, m.meeting_date, m.theme ' +
      'FROM meeting_role_assignments mra ' +
      'JOIN meeting_day_roles mdr ON mra.role_id = mdr.id ' +
      'JOIN meetings m ON mra.meeting_id = m.id ' +
      'WHERE mra.member_id = ? ORDER BY m.meeting_date DESC',
      [req.params.id]
    );

    res.json(Object.assign({}, member, { progress: progress, completions: completions, excoHistory: excoHistory, meetingRoleHistory: meetingRoleHistory }));
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Server error' });
  }
