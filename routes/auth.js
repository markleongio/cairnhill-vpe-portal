...
router.post('/login', (req, res) => {
  ...
  const member = user.member_id
    ? get('SELECT full_name, chinese_name FROM members WHERE id = ?', [user.member_id])
    : null;

  res.json({
    ok: true,
    user: { id: user.id, username: user.username, role: user.role, memberName: member?.full_name || null },
  });
});
